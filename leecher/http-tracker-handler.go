...
func GetPeersFromTrackers(torrent *entities.Torrent) ([]entities.Peer, error) {
	...
	// peers = []entities.Peer{entities.Peer{IP: net.IP([]byte{192, 168, 43, 139}), Port: 6881}}

	return peers, nil
}

// builds a tracker request url from the announcement list
func buildTrackerRequestURLs(torrent *entities.Torrent) ([]string, error) {

	// query parameters to the tracker url
	requestParams := entities.TrackerRequestParams{
		Info_hash:  torrent.InfoHash,
		Peer_id:    tools.CLIENT_ID,
		Port:       6881,
		Uploaded:   0,
		Downloaded: 0,
	...
	if tools.IsHTTPTracker(torrent.Announce) {
	...
	for _, tracker := range torrent.AnnounceList {
		...
		if tools.IsHTTPTracker(URL.String()) {
...
func getPeersFromTrackersHelper(URLs []string) ([]entities.Peer, error) {
	peers := []entities.Peer{}
...
func getPeerFromURL(URL string) ([]entities.Peer, error) {
	...
	// decode the response
	trackerResponse := entities.TrackerResponse{}
	err = bencode.NewDecoder(response.Body).Decode(&trackerResponse)
	if err != nil {
		return []entities.Peer{}, err
	}

	peers, err := entities.PeerParser([]byte(trackerResponse.Peers))
	if err != nil {
		return []entities.Peer{}, err
	}
